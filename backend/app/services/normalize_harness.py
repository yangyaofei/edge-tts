...
- 停顿单位 = **意群** (一口气能读完的意义单元), 不是单个词:
  - 意群可以包含多个词: "统计平均值" / "真实经历" / "取证环节" 各是一个意群, 内部禁止停顿
  - 停顿是给听者消化意义的时间, 放在意群边界 — 判断标准: 断开后朗读是否流畅、意义是否完整
- 例: "真实经历 压过 统计平均值 才叫特色" ✓ (真实经历/压过/统计平均值/才叫特色 四个意群)
  "真实经历 压过统计 平均值 才叫特色" ✗ ("统计平均值" 被拦腰拆断, 听者理解被打断)
  "苹果 起诉 爆料人 Jon Prosser 的 诉讼 在 取证环节 出现 拖延" ✓ (各为主谓宾块)
...
- 名词短语/动宾短语/谓语内部禁止停顿: 想离开/要做/能吃/会来/统计平均值 不能拆
